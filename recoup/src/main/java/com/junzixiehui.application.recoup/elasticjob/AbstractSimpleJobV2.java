 ...
 */
@Component
public abstract class AbstractSimpleJobV2 implements com.dangdang.ddframe.job.api.simple.SimpleJob {
    private static final Logger LOGGER = LoggerFactory.getLogger(AbstractSimpleJobV2.class);
     ...
     * @return
     */
    protected abstract List<String/*id*/> getAllShardDatas() throws Exception;
     ...
     * @return
     */
    protected abstract void processMyShardData(String myShardDataId) throws Exception;
     ...
     * @throws Exception
     */
    protected int getshardingItem(String myShardDataId, int shardingTotalCount) throws Exception {
        int shardingItem = Math.abs(myShardDataId.hashCode()) % shardingTotalCount;
        return shardingItem;
    }
  ...
  /*  @Override
    ...
    public void execute(final ShardingContext context) {
        ...
        //得到分片项
        int shardingItems = context.getShardingItem();
        if (shardingItems == 0) {
            logSB.append("该服务器未分配到分片项，放弃执行");
            LOGGER.warn(logSB.toString());
            return;
        }
        logSB.append("得到的分片项：" + shardingItems).append("|");

        //得到所有待处理任务
        List<String> allShardDatas = null;
        String exceptionMsg = "";
        try {
        ...
        //得到该实际待处理数据
        List<String> myShardDatas = Lists.newArrayList();
        int shardingTotalCount = context.getShardingTotalCount();
        try {
            for (String id : allShardDatas) {
        ...
        //交给实际任务类执行
        for (String myShardData : myShardDatas) {
            try {
